...
def add_task(todo_list, task, time, status):
        new_task = {"task": task, "time": time, "status": status}
...
def delete_task(todo_list, index):
        if 0 <= index < len(todo_list.tasks):
            ...
            if task_state in ["Nueva","Pendiente"]:
                del todo_list.tasks[index]
                print("Tarea eliminada.")
            else:
                print("No se puede eliminar la tarea. El estado no es 'Nueva'")
        else:
